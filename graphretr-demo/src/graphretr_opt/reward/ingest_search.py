...
class IngestSearchRewardAdapter:
    ...
    def _run_ingest(self, file_set, ingest_hash):
        ...
        try:
            ...
            # Candidate isolation on the shared single DB: the adapter wipes the
            # graph (MATCH (n) DETACH DELETE n) under the EX lock before this runs,
            # so each build starts from an empty graph.
